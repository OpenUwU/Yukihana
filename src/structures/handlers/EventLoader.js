...
export class EventLoader {
  ...
  async loadAllEvents() {
    try {
      ...
      const totalEvents   =Array.from(this.loadedEvents.values())
  ...
  async loadEventsByType() {
    try {
      ...
      const eventTypeEntries   =await fs.promises.readdir(this.eventsPath, { withFileTypes: true });
  ...
  async loadEventFile(filePath, eventType) {
    try {
      ...
      logger.info('EventLoader', `Loaded ${eventType} event: ${event.name || 'unnamed'}`);
    } catch (error) {
      logger.error('EventLoader', `Failed to load event: ${filePath}`, error);
    }
  }
  ...
  async reloadEvents() {
    try {
      for (const handler of this.handlers.values()) {
        if (handler.unregisterAll) {
          await handler.unregisterAll();
        }
      }
      ...
      logger.success('EventLoader', 'Successfully reloaded all events');
      return true;
    } catch (error) {
      logger.error('EventLoader', 'Failed to reload events', error);
      return false;
    }
  }

  async reloadEventType(eventType) {
    try {
      const handler   =this.handlers.get(eventType);
      if (!handler) {
        logger.error('EventLoader', `No handler found for type: ${eventType}`);
        return false;
      }

      await handler.unregisterAll();

      this.loadedEvents.delete(eventType);

      const eventTypePath   =path.join(this.eventsPath, eventType);
      if (fs.existsSync(eventTypePath)) {
        await this.recursiveLoadEvents(eventTypePath, eventType);
      }

      logger.success('EventLoader', `Reloaded all ${eventType} events`);
      return true;
    } catch (error) {
      logger.error('EventLoader', `Failed to reload ${eventType} events`, error);
      return false;
    }
  }

  async reloadSpecificEvent(eventName, eventType) {
    try {
      const handler   =this.handlers.get(eventType);
      if (!handler) {
        logger.error('EventLoader', `No handler found for type: ${eventType}`);
      ...
      await handler.unregister(eventName);

      if (this.loadedEvents.has(eventType)) {
        const events   =this.loadedEvents.get(eventType);
        const filteredEvents   =events.filter(event   => event.name   !==eventName);
        this.loadedEvents.set(eventType, filteredEvents);
      }
      ...
      logger.success('EventLoader', `Reloaded ${eventType} event: ${eventName}`);
      return true;
    } catch (error) {
      logger.error('EventLoader', `Failed to reload event: ${eventName}`, error);
      return false;
    }
  }
  ...
  getAvailableEventTypes() {
    try {
      ...
      return fs.readdirSync(this.eventsPath, { withFileTypes: true })

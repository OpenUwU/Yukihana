...
export default class nodeHandler {
    ...
    try {
      const listener = (...args) => {
        try {
          ...
          logger.error('PnodeEvent', `Error in event ${event.name}:`, error);
        }
      };
      ...
      logger.error('nodeEvent', `Failed to register player event: ${event.name}`, error);
      return false;
    }
  }
  ...
  async unregister(eventName) {
    if (this.registeredEvents.has(eventName)) {
      this.music.lavalink.nodeManager.removeListener(eventName, this.registeredEvents.get(eventName));

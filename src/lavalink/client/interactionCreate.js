...
export default class InteractionCreate {
  ...
  async execute(interaction) {
    try {
      ...
      const player   =this.client.music?.getPlayer(interaction.guild.id);
      if (!player) {
        return interaction.reply({
      ...
      if (interaction.isButton()) {
        await this._handleButtonInteraction(interaction, pm);
      }

      if (interaction.isStringSelectMenu()) {
        await this._handleSelectMenuInteraction(interaction, pm);
      }
  ...
  async _handleButtonInteraction(interaction, pm) {
  ...
  async _handleSelectMenuInteraction(interaction, pm) {
    ...
    if (selectedValue.startsWith('loop_')) {
      await this._updateSelectMenuOptions(interaction, pm);
    }
  }

  async _updateSelectMenuOptions(interaction, pm) {
    try {

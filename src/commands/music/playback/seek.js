...
class SeekCommand extends Command {
  ...
  async execute({ message, args, pm }) {
    if (!args[0]) {
      return this._reply(message, this._createErrorContainer("Please provide a time to seek to. (e.g., `2:30`, `1min 30s`, `90s`)"));
    }
  ...
  async _handleSeek(context, pm, timeString) {
    ...
    if (!track.info.isSeekable) {
      return this._reply(context, this._createErrorContainer("This track is not seekable."));
    }

    const timeMs = this._parseTime(timeString);
    if (timeMs === null) {
      return this._reply(context, this._createErrorContainer("Invalid time format. Use formats like `2:30`, `1min 30s`, or `90s`."));
    }

    if (timeMs > track.info.duration) {
      return this._reply(context, this._createErrorContainer("The specified time exceeds the track duration."));
    }
    ...
    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent("### Track Seeked")
    );
      ...
      new SectionBuilder()
        .addTextDisplayComponents(
          new TextDisplayBuilder().setContent(
            `Seeked to **${this._formatDuration(timeMs / 1000)}** / ${this._formatDuration(track.info.duration / 1000)}`
          )
        )

...
class ApplicationEmojiCommand extends Command {
  ...
  async execute({ client, message, args }) {
    ...
    switch (action) {
      ...
      default:
        return this._sendError(message, "Invalid Action", `Unknown action: \`${action}\`. Use \`appemoji\` for help.`);
    }
  }
  ...
  async _handleList(client, message) {
    try {
      ...
      logger.error("ApplicationEmojiCommand", "Failed to fetch emojis:", error);
      this._sendError(message, "Fetch Error", `Failed to fetch application emojis: ${error.message}`);
    }
  }
  ...
  async _handleCreate(client, message, args) {
    ...
    if (!name) {
      return this._sendError(message, "Missing Name", "Please provide an emoji name.\n\`appemoji create <name> <url|attachment>\`");
    }

    if (!attachment) {
      return this._sendError(message, "Missing Attachment", "Please provide an image URL or attach an image file.");
    }
    ...
    try {
      ...
      const sent = await message.reply({
        components: [this._createSuccessContainer("Emoji Created", `Successfully created emoji **${emoji.name}**`, emoji)],
        flags: MessageFlags.IsComponentsV2,
        fetchReply: true
      });
      this._setupBasicCollector(sent, message.author.id, client);
    } catch (error) {
      logger.error("ApplicationEmojiCommand", "Failed to create emoji:", error);
      this._sendError(message, "Creation Error", `Failed to create emoji: ${error.message}`);
    }
  }

  async _handleDelete(client, message, name) {
    if (!name) {
      return this._sendError(message, "Missing Name", "Please provide an emoji name to delete.\n\`appemoji delete <name>\`");
    }
    ...
    try {
      ...
      if (!emoji) {
        return this._sendError(message, "Emoji Not Found", `No emoji found with name **${name}**.`);
      }

      await client.application.emojis.delete(emoji);

      const sent = await message.reply({
        components: [this._createSuccessContainer("Emoji Deleted", `Successfully deleted emoji **${name}**`)],
        flags: MessageFlags.IsComponentsV2,
        fetchReply: true
      });
      this._setupBasicCollector(sent, message.author.id, client);
    } catch (error) {
      logger.error("ApplicationEmojiCommand", "Failed to delete emoji:", error);
      this._sendError(message, "Deletion Error", `Failed to delete emoji: ${error.message}`);
    }
  }

  async _handleEdit(client, message, name, newName) {
    if (!name || !newName) {
      return this._sendError(message, "Missing Parameters", "Please provide both current name and new name.\n\`appemoji edit <current_name> <new_name>\`");
    }
    ...
    try {
      ...
      const updatedEmoji = await client.application.emojis.edit(emoji, { name: newName });

      const sent = await message.reply({
        components: [this._createSuccessContainer("Emoji Renamed", `Successfully renamed **${name}** to **${updatedEmoji.name}**`, updatedEmoji)],
        flags: MessageFlags.IsComponentsV2,
        fetchReply: true
      });
      this._setupBasicCollector(sent, message.author.id, client);
    } catch (error) {
      logger.error("ApplicationEmojiCommand", "Failed to edit emoji:", error);
      this._sendError(message, "Edit Error", `Failed to edit emoji: ${error.message}`);
    }
  }

  async _handleInfo(client, message, name) {
    if (!name) {
      return this._sendError(message, "Missing Name", "Please provide an emoji name to get info.\n\`appemoji info <name>\`");
    }
    ...
    try {
      ...
      logger.error("ApplicationEmojiCommand", "Failed to fetch emoji info:", error);
      this._sendError(message, "Info Error", `Failed to get emoji info: ${error.message}`);
    }
  }
  ...
  _createMainContainer(client) {
    ...
    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`### ${emoji.get("folder")} Application Emoji Manager`)
    );
    ...
    const content = `**${emoji.get("info")} Application Emoji Management**\n\n` +
  ...
  _createListContainer(emojis, page = 0) {
    ...
    const endIndex = Math.min(startIndex + ITEMS_PER_PAGE, emojiArray.length);
    const pageEmojis = emojiArray.slice(startIndex, endIndex);

    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`### ${emoji.get("folder")} Application Emojis (${emojiArray.length}/50)`)
    );
    ...
    if (emojiArray.length === 0) {
      content = `**${emoji.get("cross")} No Application Emojis**\n\n` +
        `No emojis found in this application.\nUse \`appemoji create <name> <url>\` to add one.`;
    } else {
      content = `**${emoji.get("check")} Emoji List (Page ${page + 1}/${totalPages}):**\n\n`;
      
      pageEmojis.forEach((emojiItem, index) => {
        const globalIndex = startIndex + index;
        const isLast = globalIndex === pageEmojis.length - 1 && page === totalPages - 1;
        const prefix = isLast ? "└─" : "├─";
  ...
  _createInfoContainer(emojiItem) {
    ...
    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`### ${emoji.get("info")} Emoji Information`)
    );
    ...
    const content = `**${emoji.get("check")} Emoji Details:**\n\n` +
    ...
    const section = new SectionBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(content))
      .setThumbnailAccessory(new ThumbnailBuilder().setURL(emojiItem.url));
  ...
  _createSuccessContainer(title, description, emojiItem = null) {
    ...
    const section = new SectionBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(content))
      .setThumbnailAccessory(new ThumbnailBuilder().setURL(emojiItem?.url || config.assets.defaultThumbnail));
  ...
  async _createStatsContainer(client) {
    ...
    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`### ${emoji.get("info")} Application Statistics`)
    );
    ...
    try {
      ...
      const section = new SectionBuilder()
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(content))
        .setThumbnailAccessory(new ThumbnailBuilder().setURL(client.application.iconURL() || config.assets.defaultThumbnail));

      container.addSectionComponents(section);
    } catch (error) {
      const content = `**${emoji.get("cross")} Error Loading Statistics**\n\nFailed to load emoji statistics: ${error.message}`;
      const section = new SectionBuilder()
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(content))
        .setThumbnailAccessory(new ThumbnailBuilder().setURL(config.assets.defaultThumbnail));
    ...
    collector.on("collect", async (interaction) => {
      try {
        ...
        if (interaction.customId === "ae_list") {
          ...
          return this._setupListCollector(message, userId, client, emojis);
        ...
        logger.error("ApplicationEmojiCommand", "Main collector error:", error);
      }
    });
      ...
      try {
        ...
        if (interaction.customId === "ae_prev") {
          if (currentPage > 0) {
            currentPage--;
            await interaction.editReply({
              components: [this._createListContainer(emojis, currentPage)]
            });
          }
          ...
          const refreshedEmojis = await client.application.emojis.fetch();
          await interaction.editReply({
            components: [this._createListContainer(refreshedEmojis, currentPage)]
          });
          return this._setupListCollector(message, userId, client, refreshedEmojis);
        ...
        logger.error("ApplicationEmojiCommand", "List collector error:", error);
      }
    });
  ...
  _setupBasicCollector(message, userId, client) {
    ...
    collector.on("collect", async (interaction) => {
      try {
        ...
        logger.error("ApplicationEmojiCommand", "Basic collector error:", error);
      }
    });
  ...
  _setupCollectorEnd(collector, message) {
    collector.on("end", async () => {
      try {
        ...
        if (error.code !== 10008) {
          logger.error("ApplicationEmojiCommand", "Failed to disable components:", error);
        }
      }
    });
  }

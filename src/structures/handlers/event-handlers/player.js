...
export default class playerHandler {
    ...
    try {
      const listener = (...args) => {
        try {
          ...
          logger.error('PlayerEvent', `Error in event ${event.name}:`, error);
        }
      };
      ...
      logger.error('PlayerEvent', `Failed to register player event: ${event.name}`, error);
      return false;
    }
  }
  ...
  async unregister(eventName) {
    if (this.registeredEvents.has(eventName)) {
      this.music.lavalink.removeListener(eventName, this.registeredEvents.get(eventName));

...
import { LLEventsManager } from "./LLEventsManager.js";
...
export class MusicManager {
  ...
  init() {
    try {
      this.lavalink   =new LavalinkManager({
        ...
        sendToShard: (guildId, payload)   => {
          if (this.client.cluster) {
            return this.client.cluster.broadcastEval((client, context)   => {
      ...
      this.client.on("ready", async ()   => {
        logger.success("MusicManager", `🎵 ${this.client.user.tag} music system is ready!`);

        this.lavalink.init(this.client.user)

        this.eventsManager   =new LLEventsManager(this.client, this);
        await this.eventsManager.initialize();

        logger.info("MusicManager", `📊 Loaded ${this.eventsManager.getTotalEventCount()} Lavalink events`);
        this.initialized   =true;
        logger.success("MusicManager", "Initialized successfully with event handling");
      });
    } catch (error) {
      logger.error("MusicManager", "Failed to initialize music system", error);
      logger.error("MusicManager", "❌ FATAL ERROR INITIALIZING MUSIC SYSTEM:", error);
      this.initialized   =false;
    }
  }
  ...
  async createPlayer(options) {
    ...
    try {
      logger.debug("MusicManager", `🔧 DEBUG: createPlayer called with options: ${JSON.stringify(options)}`);

      const { guildId, textId, voiceId }   =this.parsePlayerOptions(options);

      logger.debug("MusicManager", `🔧 DEBUG: Parsed options: ${JSON.stringify({ guildId, textId, voiceId}) }`);
      ...
      if (existing) {
        ...
        logger.debug("MusicManager", "🔧 DEBUG: Player already exists, returning existing");
        return existing;
      }
      ...
      try {
        if (db) {
          ...
          logger.debug("MusicManager", `🔧 DEBUG: Got default volume from database: ${playerVolume}`);
          logger.debug("MusicManager", `Using default volume ${playerVolume} for guild ${guildId}`);
        } else {
          logger.debug("MusicManager", "⚠️ DEBUG: Database or getDefaultVolume method not available, using fallback");
          logger.warn("MusicManager", "Database not available, using fallback volume 100");
        }
      } catch (error) {
        logger.debug("MusicManager", `❌ DEBUG: Error getting default volume: ${error.message}`);
      ...
      if (isNaN(playerVolume) || playerVolume < 1 || playerVolume > 100) {
        logger.debug("MusicManager", `⚠️ DEBUG: Invalid volume, resetting to 100: ${playerVolume}`);
        logger.warn("MusicManager", `Invalid volume ${playerVolume}, using 100`);
        playerVolume   =100;
      }

      logger.debug("MusicManager", `🔧 DEBUG: Final volume to use: ${playerVolume}`);
      ...
      logger.debug("MusicManager", `🔧 DEBUG: Player config: ${JSON.stringify(playerConfig)}`);

      const player   =await this.lavalink.createPlayer(playerConfig);

      if (!player) {
        logger.debug("MusicManager", "❌ DEBUG: Lavalink createPlayer returned null/undefined");
        logger.error("MusicManager", `Failed to create player for guild ${guildId}`);
        return null;
      }

      logger.debug("MusicManager", "🔧 DEBUG: Player created successfully, attempting connection");

      if (!player.connected) {
        logger.debug("MusicManager", "🔧 DEBUG: Player not connected, connecting...");
        await player.connect();
        logger.debug("MusicManager", "🔧 DEBUG: Player connected");
      }

      logger.success("MusicManager", `Player created and connected successfully for guild ${guildId} with default volume ${playerVolume}`);
      logger.debug("MusicManager", "✅ DEBUG: Player creation completed successfully");
      return player;
    } catch (error) {
      logger.debug("MusicManager", `❌ DEBUG: Error in createPlayer: ${error.message}`);
      logger.error("MusicManager", `Error creating player for guild ${options?.guildId || 'unknown'}: ${error.message}`);
  ...
  reloadEvent(category, eventName) {
    if (this.eventsManager) {
      return this.eventsManager.reloadEvent(category, eventName);
    }
    return false;
  }
  ...
  getEventInfo() {
    if (this.eventsManager) {
      return this.eventsManager.getEventInfo();
    }
    return {
      totalEvents: 0,
      categories: { client: 0, player: 0, track: 0, node: 0 },
      events: { client: [], player: [], track: [], node: [] }
    };
  }
}

...
class RemoveTrackCommand extends Command {
		constructor() {
				super({
						...
						slashData: {
								...
								options: [
										{ name: "playlist", description: "The ID or name of the playlist", type: 3, required: true },
										{ name: "positions", description: "Track numbers or ranges to remove (e.g., 5, 8-10)", type: 3, required: true },
								],
						},
				});
		}
		...
		async execute({ client, message, args }) {
				if (args.length < 2) return message.reply({ embeds: [this._createUsageEmbed()] });
		...
		async _handleRemove(user, context, query, positionsStr) {
				...
				if (!playlist) return this._reply(context, this._createErrorContainer(`Playlist not found for query: \`${query}\``));
				if (playlist.tracks.length === 0) return this._reply(context, this._createErrorContainer("This playlist is empty."));

				const indicesToRemove = this._parseTrackIndices(positionsStr, playlist.tracks.length);
				if (indicesToRemove.length === 0) {
						return this._reply(context, this._createErrorContainer("Invalid or out-of-range track numbers provided."));
				}
				...
				// Sort indices descending to avoid shifting issues while removing
		...
		_createErrorContainer(message) {
				return new ContainerBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(`${emoji.get("cross")} **Error**\n${message}`));
		}

		_createUsageEmbed() {
				// Simplified usage for non-container reply
				return {
						color: 0xff5555,
						title: "Incorrect Usage",
						description: "You need to specify a playlist and the track numbers/ranges to remove.",
						fields: [
								{ name: "Usage", value: "`remove-track <playlist_name_or_id> <positions>`" },
								{ name: "Examples", value: "• `remove-track MyPlaylist 5`\n• `remove-track MyPlaylist 3,7`\n• `remove-track MyPlaylist 5-10`" }
						]
				};
		}

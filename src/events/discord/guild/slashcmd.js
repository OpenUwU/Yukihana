import { InteractionType, ContainerBuilder, TextDisplayBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, MessageFlags, SeparatorBuilder, SeparatorSpacingSize } from 'discord.js';
...
import { canUseCommand, getMissingBotPermissions, inSameVoiceChannel, hasPremiumAccess } from '#utils/permissionUtil';
...
async function _sendPremiumError(interaction, type) {
  ...
  const container   =new ContainerBuilder()
    .addTextDisplayComponents(new TextDisplayBuilder().setContent(`**${type} Required**`))
    .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
    .addTextDisplayComponents(new TextDisplayBuilder().setContent(`This command requires a **${type}** subscription.\nContact the bot owner for access.`))
    .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
  ...
  try {
    ...
    logger.error('InteractionCreate', 'Failed to send premium error reply.', error);
  }
}
...
async function handleChatInputCommand(interaction, client) {
  ...
  if (!commandToExecute) {
    logger.warn('InteractionCreate', `No command file found for interaction: /${interaction.commandName}`);
  ...
  try {
    ...
    if (!canUseCommand(interaction.member, commandToExecute)) {
      return _sendError(interaction, 'Insufficient Permissions', 'You do not have the required permissions to use this command.');
    }
    ...
    if (commandToExecute.permissions?.length > 0) {
      ...
      if (missingBotPerms.length > 0) {
        return _sendError(interaction, 'Missing Bot Permissions', `I am missing the following permissions to run this command: \`${missingBotPerms.join(', ')}\``);
      }
    }
    ...
    if (commandToExecute.playerRequired && !player) {
      return _sendError(interaction, 'No Player Active', 'There is no music player active in this server.\nUse `/play` to start one.');
    }
    if (commandToExecute.playingRequired && (!player || !player.queue.current)) {
      return _sendError(interaction, 'Nothing Playing', 'There is no track currently playing.\nUse `/play` to start a song.');
    }
    ...
    logger.error('InteractionCreate', `Error executing slash command '${commandToExecute.slashData.name}'`, error);

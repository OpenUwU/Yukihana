...
export default class TrackStart {
  ...
  async execute(player, track, payload) {
    try {
      ...
      try {
        const buffer   =await this.musicCard.createMusicCard(track, 0);
        const attachment   =new AttachmentBuilder(buffer, { name: 'yukihana-nowplaying.png' });
        const components   =this._createControlComponents();

        message   =await EventUtils.sendPlayerMessage(this.client, player, {
          files: [attachment],
          components,
        });
      } catch (cardError) {
      ...
      try {
        ...
        const message   =await EventUtils.sendPlayerMessage(this.client, player, {
          content: `🎵 **Now Playing**\n**${title}** by **${author}**`,
          components: this._createControlComponents(),
        });

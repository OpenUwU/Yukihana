...
export class SpotifyManager {
  ...
  async getAccessToken() {
    try {
      ...
      if (!this.clientId || !this.clientSecret) {
        logger.error('SpotifyManager', 'Spotify credentials not configured');
        return null;
      }
  ...
  async apiRequest(endpoint, method = 'GET') {
    try {
      ...
      logger.error('SpotifyManager', `API request error: ${endpoint}`, error);
      return null;
    }
  }
  ...
  async fetchUserPlaylists(profileUrl) {
    try {
      ...
      return data.items
        .filter(playlist => playlist.public && playlist.tracks?.total > 0)
        .map(playlist => ({
          id: playlist.id,
          name: playlist.name,
      ...
      logger.error('SpotifyManager', 'Error fetching user playlists', error);
      return null;
    }
  }
  ...
  async fetchPlaylistTracks(playlistId, limit = 100) {
    try {
      ...
      logger.error('SpotifyManager', 'Error fetching playlist tracks', error);
      return null;
    }
  }

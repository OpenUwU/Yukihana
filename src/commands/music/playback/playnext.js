...
class PlayNextCommand extends Command {
  constructor() {
    super({
      name: "playnext",
      description: "Add music to play next in queue",
      usage: "playnext <query> [--src yt/am/sp/sc/dz]",
      aliases: ["pnext", "playupcoming"],
      category: "music",
      examples: [
  ...
  async _handlePlayNextRequest({ client, guildId, query, source, requester, pm }) {
    try {
      ...
      if (searchResult.loadType   ==="playlist") {
        ...
        return this._handlePlayNextSingleTrack(pm, searchResult.tracks[0], guildId, requester.id);
      }
    } catch (error) {
  ...
  async _handlePlayNextSingleTrack(playerManager, track, guildId, userId) {
    ...
    if (wasEmpty) {
      ...
      return { success: true, type: "playing_next_empty", track };
    } else {
      const premiumStatus   =this._getPremiumStatus(guildId, userId);
      return { success: true, type: "queued_next", track, premiumStatus };
    }
  }
  ...
  async _handlePlayNextPlaylist(playerManager, searchResult, guildId, userId) {
    ...
    for (let i   =tracksToAdd.length - 1; i >= 0; i--) {
      await playerManager.addTracks(tracksToAdd[i], 0);
    }
    ...
    if (wasEmpty) {
      ...
      if (limitWarning) {
        return {
          success: true,
          type: "playlist_playing_next_partial",
      ...
      return { success: true, type: "playlist_playing_next", playlist: searchResult.playlist, tracks: tracksToAdd };
    } else {
      const premiumStatus   =this._getPremiumStatus(guildId, userId);
      if (limitWarning) {
        return {
          success: true,
          type: "playlist_queued_next_partial",
          playlist: searchResult.playlist,
          tracks: tracksToAdd,
          totalTracks: tracks.length,
          limitWarning,
      ...
      return { success: true, type: "playlist_queued_next", playlist: searchResult.playlist, tracks: tracksToAdd, premiumStatus };
    }
  }
  ...
  _createLoadingContainer(query) {
    ...
    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent("Music Search - Play Next")
    );
    ...
    container.addSectionComponents(
      new SectionBuilder()
        .addTextDisplayComponents(
          new TextDisplayBuilder().setContent(`Searching to play next: ${query}`)
        )
  ...
  _createSuccessContainer(result) {
    ...
    if (result.type   ==="playing_next_empty" || result.type   ==="queued_next") {
      const { track, premiumStatus }   =result;
      const title   =result.type   ==="playing_next_empty" ? "Now Playing" : "Added to Play Next";
      ...
      if (result.type   ==="queued_next" && premiumStatus) {
        container.addSeparatorComponents(
          new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small)
        );

        const statusText   =premiumStatus.hasPremium
          ? "Premium Queue Active | Will play next"
          : `Free Queue Active | Will play next | Upgrade for ${config.queue.maxSongs.premium} song limit`;

        container.addSectionComponents(
          new SectionBuilder()
            .addTextDisplayComponents(
              new TextDisplayBuilder().setContent("Queue Position: Next"),
      ...
      if (result.type   ==="playlist_playing_next") {
        title   ="Playing Playlist";
        description   =`Added ${trackCount} tracks to play next`;
      } else if (result.type   ==="playlist_playing_next_partial") {
        title   ="Playing Playlist";
        description   =limitWarning;
      } else if (result.type   ==="playlist_queued_next_partial") {
      ...
      if ((result.type   ==="playlist_queued_next" || result.type   ==="playlist_queued_next_partial") && premiumStatus) {
        container.addSeparatorComponents(
          new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small)
        );

        const statusText   =premiumStatus.hasPremium
          ? "Premium Queue Active | Playing next"
          : `Free Queue Active | Playing next | Upgrade for ${config.queue.maxSongs.premium} song limit`;

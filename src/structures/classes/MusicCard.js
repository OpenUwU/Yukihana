...
export default class MusicCard {
  ...
  registerFonts() {
    try {
      const fontPaths   =[
        ...
        { path: join(process.cwd(), "src", "fonts"), context: "command" },
        { path: join(process.cwd(), "fonts"), context: "root" },
        { path: join(process.cwd(), "assets", "fonts"), context: "assets" },
      ];
  ...
  async drawArtwork(ctx, track, x, y, size) {
    ...
    try {
      ...
      positions.forEach((pos)   => {
        this.createFrostSnowflake(ctx, x + pos.x, y + pos.y, pos.size, 0.3);
      });
    }
    ctx.restore();
  ...
  async createMusicCard(track, position   =0, options   ={}) {
    ...
    ctx.fillText("Yukihana", width - margin - 40, bottomY);

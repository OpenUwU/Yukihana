...
export default class QueueEnd {
  ...
  async execute(player, track, payload) {
    try {
      ...
      if (autoplayEnabled && lastTrack) {
        ...
        try {
          await this._handleAutoplay(player, lastTrack);
          return;
        } catch (autoplayError) {
      ...
      const completionMessage   =await EventUtils.sendPlayerMessage(this.client, player, {
        content: is247Mode
      ...
      this._clearStoredMessageIds(player);
  ...
  async _handleAutoplay(player, lastTrack) {
    if (!lastTrack?.info) {
      throw new Error('No valid last track for autoplay');
    }

    const autoplayMessage   =await EventUtils.sendPlayerMessage(this.client, player, {
    ...
    if (!this._isSpotifySource(lastTrack)) {
      try {
        const searchQuery   =`${lastTrack.info.author} ${lastTrack.info.title}`;
        const spotifyResult   =await this.client.music.search(searchQuery, {
          source: "spsearch"
        });

        if (spotifyResult?.tracks?.length > 0) {
          targetTrack   =spotifyResult.tracks[0];
          const originalSource   =this._getTrackSource(lastTrack);
          logger.debug('QueueEnd', `Found Spotify version for autoplay (original: ${originalSource}): ${targetTrack.info.title}`);
        } else {
          logger.debug('QueueEnd', `No Spotify version found for ${lastTrack.info.title}, using original track from ${this._getTrackSource(lastTrack)}`);
        }
      } catch (spotifyError) {
        logger.warn('QueueEnd', `Could not find Spotify version for ${this._getTrackSource(lastTrack)} track, using original:`, spotifyError);
      }
    } else {
      logger.debug('QueueEnd', 'Track is already from Spotify, using original for autoplay');
    }

    const recommendations   =await this._fetchRecommendations(targetTrack);
    ...
    const premiumStatus   =this._getPremiumStatus(player.guildId, autoplayUserId);
    ...
    for (const rec of tracksToAdd) {
      try {
        ...
        if (rec.trackInfo) {
          ...
          searchResult   =await this.client.music.search(query, {
            source: "spsearch"
          });
        }
  ...
  async _fetchRecommendations(track) {

...
>>>>>>> a1677a6 (fix)
	...
	async createPlayer(options) {
		if (!this.initialized) {
			logger.error(
				'MusicManager',
				'Cannot create player – not initialized',
			);
			return null;
		}
		...
		try {
			logger.debug(
				'MusicManager',
				`🔧 DEBUG: createPlayer called with options: ${JSON.stringify(
					options,
				)}`,
			);

			const { guildId, textId, voiceId } =
				this.parsePlayerOptions(options);

			logger.debug(
				'MusicManager',
				`🔧 DEBUG: Parsed options: ${JSON.stringify({
					guildId,
					textId,
					voiceId,
				})}`,
			...
			if (!guildId || !textId || !voiceId) {
				logger.error(
					'MusicManager',
					'Missing IDs for player creation',
					{
						guildId,
						textId,
						voiceId,
			...
			logger.error(
				'MusicManager',
				`Error creating player for guild ${
					options?.guildId || 'unknown'
				}: ${error.message}`,
			);

...
class UserPrefixCommand extends Command {
  ...
  _buildUIManagementContainer(username, prefixes = [], actionMessage = null) {
    ...
    container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.get('info')} Personal Prefix Management`));
    container.addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small));

    const prefixList = prefixes.length > 0 ?
      prefixes.map(p => `\`${p}\``).join(' ') :
      "You have no custom prefixes.";
    let content = actionMessage ? `**${actionMessage}**\n\n` : '';
    content += `**Your Prefixes (${prefixes.length}/${USER_PREFIX_LIMIT}):**\n${prefixList}\n\n*These prefixes work for you in any server I'm in.*`;
  ...
  async _addPrefix(userId, newPrefix) {
    if (!newPrefix || newPrefix.trim() === '') return { success: false, message: "Please provide a valid prefix." };
    const trimmedPrefix = newPrefix.trim();
    if (trimmedPrefix.length > 5) return { success: false, message: "Prefix is too long (max 5 characters)." };
    const currentPrefixes = db.getUserPrefixes(userId);
    if (currentPrefixes.length >= USER_PREFIX_LIMIT) return { success: false, message: `Prefix limit of ${USER_PREFIX_LIMIT} reached.` };
    if (currentPrefixes.includes(trimmedPrefix)) return { success: false, message: `The prefix \`${trimmedPrefix}\` already exists.` };

    const newPrefixes = [...currentPrefixes, trimmedPrefix];
    db.setUserPrefixes(userId, newPrefixes);
    return { success: true, message: `Successfully added prefix: \`${trimmedPrefix}\``, prefixes: newPrefixes };
  }
  ...
  _setupCollector(message, userId, username) {
    const collector = message.createMessageComponentCollector({ filter: (i) => i.user.id === userId, time: 300_000 });
    ...
    collector.on('collect', async (interaction) => {
        try {
            ...
            if (interaction.customId === 'up_add') {
                const modal = new ModalBuilder().setCustomId('up_add_modal').setTitle('Add a Personal Prefix')
                    .addComponents(new ActionRowBuilder().addComponents(
                        new TextInputBuilder().setCustomId('new_prefix_input').setLabel("New Prefix (max 5 chars)").setStyle(TextInputStyle.Short).setRequired(true).setMaxLength(5)
                    ));
                ...
                try {
                    ...
                    await interaction.editReply({ components: [this._buildUIManagementContainer(username, prefixes, actionMessage)] });
                } catch (error) { return; }
            } else if (interaction.customId === 'up_remove_all') {
                await interaction.deferUpdate();
                db.setUserPrefixes(userId, []);
                prefixes = [];
                actionMessage = `${emoji.get('check')} All custom prefixes have been removed.`;
                await interaction.editReply({ components: [this._buildUIManagementContainer(username, prefixes, actionMessage)] });
                ...
                actionMessage = `${emoji.get('check')} Removed: ${valuesToRemove.map(p => `\`${p}\``).join(', ')}`;
                await interaction.editReply({ components: [this._buildUIManagementContainer(username, prefixes, actionMessage)] });
            }
        } catch (error) {

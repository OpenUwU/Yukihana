...
class PrefixCommand extends Command {
        ...
        options: [
          {
            name: "set",
            description:
              "Set a new prefix. This will overwrite it for standard servers.",
            type: 3,
            required: false,
            max_length: 5,
          },
  ...
  async _setPrefix(ctx, newPrefix, isPremium, isInteraction) {
    if (newPrefix.length > 5) {
      return this._sendError(
        ctx,
        "Error",
        "Prefix is too long. Maximum 5 characters allowed.",
      );
    }
    ...
    if (isPremium) {
      if (prefixes.includes(newPrefix)) {
        return this._sendError(
          ctx,
          "Prefix Exists",
          `The prefix \`${newPrefix}\` is already set for this server.`,
        );
      }
      if (prefixes.length >= PREMIUM_PREFIX_LIMIT) {
        return this._sendError(
          ...
          `Premium servers can have a maximum of ${PREMIUM_PREFIX_LIMIT} prefixes.`,
        );
      }
    ...
    const replyOptions = this._createSuccessResponse(
      "Prefix Updated",
      updateMessage,
      newPrefix,
    );
  ...
  _createPremiumContainer(prefixes) {
    ...
    const section = new SectionBuilder()
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `**Current Prefixes (${prefixes.length}/${PREMIUM_PREFIX_LIMIT}):**\n${prefixList}`,
        ),
      )
      .setThumbnailAccessory(
        new ThumbnailBuilder().setURL(config.assets.defaultThumbnail),

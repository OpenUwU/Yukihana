...
class QueueCommand extends Command {
  ...
  async execute({ client, message, args }) {
    ...
    return this._handleQueue(client, message.guild.id, message, isNaN(page) ? 1 : page);
  }
  ...
  async _handleQueue(client, guildId, context, page) {
    ...
    const message   =await this._reply(context, this._buildQueueContainer(player, page, guildId, userId));
    if (message) {
  ...
  _buildQueueContainer(player, page, guildId, userId) {
    ...
    container.addSectionComponents(new SectionBuilder()
      .addTextDisplayComponents(
        ...
        new TextDisplayBuilder().setContent(`[${current.info.title}](${current.info.uri}) - \`${this._formatDuration(current.info.duration)}\``)
      )
    ...
    container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`*${queueStatus}*`));
    ...
    if (tracks.length > 0) {
      ...
      container.addTextDisplayComponents(new TextDisplayBuilder().setContent(upNextText));
      ...
      paginatedTracks.forEach((track, index)   => {
        ...
        container.addSectionComponents(
          new SectionBuilder()
          .addTextDisplayComponents(
            new TextDisplayBuilder().setContent(`**${trueIndex + 1}.** [${track.info.title}](${track.info.uri})`),
            new TextDisplayBuilder().setContent(`*Added by ${track.requester?.username || 'Unknown'} | ${this._formatDuration(track.info.duration)}*`)
          )
          .setThumbnailAccessory(new ThumbnailBuilder().setURL(track.info.artworkUrl || config.assets.defaultTrackArtwork))
        );
      });

      container.addActionRowComponents(this._createRemoveMenu(paginatedTracks, startIndex, player.guildId));
      container.addActionRowComponents(this._createPaginationButtons(page, maxPages, player.guildId));
    } else {
      container.addTextDisplayComponents(new TextDisplayBuilder().setContent("*The queue is empty.*"));
    }
  ...
  async _setupCollector(message, client, guildId, initialPage) {
    ...
    collector.on("collect", async (interaction)   => {
      ...
      if (action   ==='page') {
        ...
        const indicesToRemove   =interaction.values.map(v   => parseInt(v, 10)).sort((a, b)   => b - a);
        ...
        const newTotalPages   =Math.ceil(player.queue.tracks.length / TRACKS_PER_PAGE) || 1;
      ...
      try {
        ...
        const disabledRows   =currentMessage.components.map(row   => {
          ...
          row.components.forEach(component   => {
            let newComp;
            if (component.type   ===2) {
              newComp   =ButtonBuilder.from(component).setDisabled(true);
            } else if (component.type   ===3) {
              newComp   =SelectMenuBuilder.from(component).setDisabled(true);
            }
            if(newComp) newRow.addComponents(newComp);
          });
          return newRow;
        });

        await currentMessage.edit({ components: disabledRows });
      } catch (e) {
        if (e.code   !==10008) logger.error("QueueCommand", "Failed to disable queue components:", e);
      }
    });
  }
